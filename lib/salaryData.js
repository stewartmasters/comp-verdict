 ...
 *   Destatis VSE — Germany Federal Statistical Office, Verdienststrukturerhebung (dl-de/by-2-0)
 *   Eurostat SES — European earnings structure survey (cross-check)
...
import { getRealBenchmark } from './real-benchmarks.js'
...
export function getBenchmark(role, city, yoe) {
  ...
  // ── Try real statistical data first ─────────────────────────────────────
  const real = getRealBenchmark(role, city)
  if (real) {
    const bandData = real.bands[band]
    if (bandData) {
      const { p25, p50, p75 } = bandData
      const p10 = Math.round(p25 - 0.85 * (p50 - p25))
      const p90 = Math.round(p75 + 0.80 * (p75 - p50))
      return { p10, p25, p50, p75, p90, band, fallbackCity: null, symbol: cityMeta.symbol, fromRealData: true }
    }
  }

  // ── Multiplier fallback for roles/cities not yet in the real dataset ────
  ...
  return { p10, p25, p50, p75, p90, band, fallbackCity, symbol: cityMeta.symbol, fromRealData: false }
}
 ...
 * @returns {'high'|'med'}
 */
export function getConfidenceLevel(city, role) {
  const real = role ? getRealBenchmark(role, city) : null
  if (real) return 'high'  // backed by official statistical sources
